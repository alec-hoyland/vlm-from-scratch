...
class DecoderLanguageModel(nn.Module):
    def __init__(
        self, n_embed, image_embed_dim, vocab_size, num_heads, n_layer, use_images=False
    ):
        super().__init__()
        self.use_images = use_images
        self.token_embedding_table = nn.Embedding(vocab_size, n_embed)
        self.position_embedding_table = nn.Embedding(1000, n_embed)
        if use_images:
            self.image_projection = MultiModalProjector(n_embed, image_embed_dim)
        self.blocks = nn.Sequential(
            *[Block(n_embed, num_heads, is_decoder=True) for _ in range(n_layer)]
        )
